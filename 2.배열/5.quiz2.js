...
while (true) {
  //삭제할 멤버 이름 입력
  var deleteName = prompt(
    `현재 멤버: [${currentMember}]\n삭제할 이름을 입력하세요.`
  );
  if (currentMember.includes(deleteName)) {
    var deleteNameNum = currentMember.indexOf(deleteName);
    //삭제한 멤버 제외 결과
    currentMember.splice(deleteNameNum, 1);
    alert(`남은멤버:[${currentMember}]`);
  } else {
    //잘못된 이름
